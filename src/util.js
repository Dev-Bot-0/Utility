...
(function () {
        ...
        var newWorker = window.Worker = class Worker extends oldWorker {
            constructor(twitchBlobUrl, options) {
                ...
                if (newWorker.toString() !== window.Worker.toString()) {
                    console.log('[AdBlock] Multiple adblockers installed. Skipping Worker hook.');
    ...
    function hookWorkerFetch() {
        console.log('[AdBlock] Twitch Worker hooked.');
        var realFetch = fetch;
    ...
    async function processM3U8(url, textStr, realFetch, playerType) {
        ...
        if (haveAdTags) {
                    ...
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        if (line.startsWith('#EXTINF') && lines.length > i + 1) {
                            if (!line.includes(',live') && !streamInfo.RequestedAds.has(lines[i + 1])) {
                                ...
                                //console.log('[AdBlock] Fetching .ts file');
            ...
            if (streamInfo && streamInfo.Urls) {
                for (const [resUrl, resInfo] of Object.entries(streamInfo.Urls)) {
                    if (resUrl == url) {
                        ...
                        //console.log(`[AdBlock] Current resolution: ${resInfo.Resolution}`);
                        break;
                    }
                }
            }
            ...
            if (WasShowingAd) {
                console.log('[AdBlock] Finished blocking ads.');
                WasShowingAd = false;
    ...
    function doTwitchPlayerTask(isPausePlay, isCheckQuality, isCorrectBuffer, isAutoQuality, setAutoQuality) {
        ...
        try {
            ...
            if (!reactRootNode) {
                console.log('[AdBlock] Could not find React root node.');
                return;
            }
    ...
    if (window.Worker.toString().includes('twitch')) {
        console.log('[AdBlock] Twitch Worker is already hooked.');
    } else {
